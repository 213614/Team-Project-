...
<div class="wrapAll">
	...
	<input type="hidden" name="like" value="${like}" />
		...
		<div class="list_container">
			...
			<span class="category">
		    	<select id="category" name="category" onchange="if(this.value) location.href=(this.value);">
		         	<option value="/list.do?category=${category}" <c:if test="${orderby == 'r'}">selected</c:if> >최신순</option>
		         	<option value="/list.do/popularCateg?category=${category}" <c:if test="${orderby == 'p'}">selected</c:if> >인기순</option>
		         	<option value="/list.do/likeCateg?category=${category}" <c:if test="${orderby == 'l'}">selected</c:if> >좋아요순</option>
		  	 	</select>
		    </span><!-- 최신순/인기순/좋아요순 카테고리 끝 -->
			</c:if>
